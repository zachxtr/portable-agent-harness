...
compatibility: Portable agent harness (.agents/joshua/). 
...
- **TODO grooming** — prune backlog, absorb scoped items into a WIP (when Zach asks or at close-out)
...
| **yield** | Zach validates as first user — don’t stack unvalidated features |
...
Legacy archived WIPs may still have old `wip:` YAML — leave as-is; do not extend that schema.
...
3. **Product direction (Zach, YYYY-MM-DD)** — locked decisions; principles; out-of-scope
4. **Spec / phases / checklists** — forward-looking plan
5. **Open questions** — awaiting Zach
6. **Zach's Thoughts** — raw capture (see below)

**Active body = target design.** Avoid debate archaeology in spec sections; use **History** only when needed.

---

## Zach's Thoughts (required section)
...
New raw note from Zach goes here as a plain line.
```

**Rules**

- Zach adds **plain lines** — fragments OK
- Joshua **never** adds synthesized rows here when folding into the body
- Absorbed thought → prefix **same line** with `DONE:`
- Open questions → **Open questions** section (unless Zach wrote them here first)

**Product direction** = polished output of Zach's Thoughts + conversation (date-stamped when locked).
...
**Goal: prune noise**, not catalog everything in a WIP. Run when Zach asks, at WIP close-out, or when TODO feels stale.
...
3. Body: **Status: design** + Product direction stub or Zach's Thoughts
...
- Grooming is documentation unless Zach asks for code in the same session
- FAQ retired — product knowledge = Platform Guide
...
| `memory/MEMORY.md` | WIP index (primary pickup) |
