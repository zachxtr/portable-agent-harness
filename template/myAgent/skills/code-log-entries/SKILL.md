...
description: "Maintain the project development code log. Use when the user asks to document, update, or create a code log entry. Writes client-readable progress summaries into dated markdown files under .agents/code-log/. Each session entry captures what was completed, what is in progress, and what the next steps are — in plain language suitable for a non-technical audience."
license: MIT — Copyright © 2026 Data Finn (datafinn.com)
compatibility: Requires .agents/code-log/ and git history in the host repository.
allowed-tools: read_file write_file shell_command glob
---

## Description

Manages the {{PROJECT_NAME}} development progress log. Produces dated session entries readable by both the client and the agent (as session memory). Does not write technical developer notes — those belong in git commits and developer docs.
...
- File lives at: `.agents/code-log/code-log-YYYYMMDD.md` (relative to repo root)
...
- Good: "Set up the development agent harness — the agent can now maintain context between sessions"
- Bad: "Created SESSION.md, IDENTITY.md, SOUL.md, USER.md, MEMORY.md in .agents/myAgent/"
...
```markdown
# {{PROJECT_NAME}} Code Log — YYYY-MM-DD

> Development progress log for {{PROJECT_NAME}}. Each session captures completed work, current progress, and next steps in language suitable for both stakeholders and development agents.
```
