...
compatibility: Policy Command agent harness (.agents/joshua/). Portable to other projects using the Full CRY! agent harness pattern.
...
Methodology: `memory/full-cry-sdlc/Full_CRY_Overview.md` § Portable Agent Harness. Session hook: load this skill for WIP work (`SESSION.md` § Session Start step 7).

---

## When to use this skill

- Zach asks to groom TODO against a WIP, or to “pull relevant bits from TODO”
...
| **yield** | **Yield** | Zach uses the system as first user — **do not stack features on unvalidated work** | `code-log/` entry; `MEMORY.md` status; set `next_yield` in frontmatter |
...
| **Domain concepts** | `## Policy Profile`, `## Briefs`, … | Ongoing notes by product area; may become WIPs | Prose + bullets OK; WIP pointers in italics at section bottom |
...
| **`memory/TODO.md` — domain concepts** | Ongoing product/engineering notes by area (Briefs, Policy Assistant, RAP, …) |
| **`memory/TODO.md` — Potential new functionality** | Unscoped future themes — promote to a WIP when scheduled |
| **`memory/TODO.md` — Ideas dump** | Raw thoughts — not groomed; agents do not pull from here into WIPs without Zach |
...
Run when Zach asks to clean TODO against a WIP, or when closing a grooming session on a `wip_*.md` file.
...
| **Raw / unscoped** | Leave in **Ideas dump** — do not pull into WIP without Zach |
...
- WIP index row in `MEMORY.md` § Topic index
...
Customize only the **topic label** in brackets (e.g. “Platform Guide and landing hero”, “rolling memory and INTERPRET handoff”).

---

## Starting a new WIP (Design)

1. Confirm topic with Zach if ambiguous
...
Before Zach yields or before session close on WIP work:
...
- **FAQ is retired** — product knowledge = **Platform Guide** (`wip_platform_guide_landing_hero.md`); do not recreate FAQ sections in TODO
...
- Grooming is **documentation only** unless Zach explicitly asks for code changes in the same session
...
| `memory/CODING_PRINCIPLES.md` | Coding preferences for this project, Acts as a refactor alignment guide; harness `SKILL.md` file shape |
